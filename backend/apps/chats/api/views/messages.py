...
from apps.chats.api.serializers import ChatMessageSerializer
...
class ChatMessagesView(BaseView):
    ...
    def post(self, request, chat_id):
        ...
        if not body and not file and not audio:
            raise ValidationError("Nenhum parametro informado")
        ...
        chatmessage_data = ChatMessageSerializer(chatmessage).data
        
        # Sending new message to chat
        socket.emit('update_chat_message', {
            'type': 'create',
            'message': chatmessage_data,
            'query': {
                'chat_id': chat_id,
            }
        })
        
        # Update chat viewed at
